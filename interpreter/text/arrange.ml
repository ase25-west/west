...
let omit_if omit s = if s = omit then "" else (" " ^ s)
...
let table off i tab =
  ...
  Node ("table $" ^ nat (off + i) ^ omit_if "i32" (addr_type at) ^ " " ^ limits nat64 lim,
    atom ref_type t :: list instr tinit.it
  )

let memory off i mem =
  let {mtype = MemoryT (at, lim)} = mem.it in
  Node ("memory $" ^ nat (off + i) ^ omit_if "i32" (addr_type at) ^ " " ^ limits nat64 lim, [])
...
let assertion mode ass =
  ...
  (* For West *)
  | AssertUninstantiableInlined (def, re) ->
    [Node ("assert_trap", [definition mode false None def; Atom (string re)])]
