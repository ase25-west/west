...
let check_timeout () =
  match !timeout with
  | None -> ()
  | Some timeout -> if Sys.time () -. !time > timeout then raise Exception.Timeout
let init_timeout () = time := Sys.time ()
let set_timeout t = timeout := t

let step_cnt = ref 0
let step_limit = ref None
let check_step_cnt () =
  step_cnt := !step_cnt + 1;
  match !step_limit with
  | None -> ()
  | Some limit -> if !step_cnt > limit then raise Exception.Timeout
let init_step_cnt () = step_cnt := 0
let set_step_limit lim = step_limit := lim
