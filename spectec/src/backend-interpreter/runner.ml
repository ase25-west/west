...
let flag_silent = ref false
let _print_endline s = if !flag_silent then () else print_endline s
...
let run_command command =
  ...
  let result =
    ...
    | Exception.Error (at, msg, step) ->
      let msg' = msg ^ " (interpreting " ^ step ^ " at " ^ Source.string_of_region at ^ ")" in
      command.at |> string_of_region |> _print_endline;
      (* error_interpret at msg' *)
      _print_endline ("- Test failed at " ^ string_of_region command.at ^
        " (" ^ msg' ^ ")");
      fail
...
let parse_file name parser_ file =
  Printf.sprintf "===== %s =====" name |> _print_endline;
  log "===========================\n\n%s\n\n" name;
  ...
  with e ->
    ...
    _print_endline ("- Failed to parse " ^ name ^ "\n");
  ...
  | path :: args when Sys.file_exists path ->
    ...
    if Sys.is_directory path then (
      if !num_parse_fail <> 0 then
        _print_endline ((string_of_int !num_parse_fail) ^ " parsing fail");
