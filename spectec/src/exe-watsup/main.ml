...
[
  ...
  "--test:out", Arg.Set_string Backend_test.Flag.out, " Set the output directory of test generation, default to `out`";
  "--test:seed", Arg.Set_int Backend_test.Flag.seed, "  Generate specific test case";
  "--test:n", Arg.Set_int Backend_test.Flag.n, "  Set the number of test cases to generate, default to 100";
  "--test:time", Arg.Set_int Backend_test.Flag.time, "  Set time limit of fuzzing in seconds, default to none";
  "--test:log", Arg.Set_int Backend_test.Flag.log, "  Set the logging level, default to 3 (warn)";
  "--test:swarm", Arg.Set Backend_test.Flag.swarm, "  Enable swarm testing";
  "--test:clean", Arg.Set Backend_test.Flag.clean, "  Remove generated wast file";
  "--test:approach", Arg.String (fun s -> Backend_test.Flag.set_approach s), "  Set top-down or bottom-up approach (default: bottom-up)";
  "--test:module-num", Arg.Set_int Backend_test.Flag.module_num, "  Set number of modules per test (default: 1)";
  "--test:target", Arg.String (fun s -> Backend_test.Flag.set_target s), "  Set engine/runtime target (default: ref-interpreter)";
  "--test:seeds", Arg.Set_string Backend_test.Flag.seeds, "  Give specific seeds to run as a file";
  "--test:cov", Arg.Set Backend_test.Flag.cov, "  Print the coverage of the generated test and official test";
  ...
  try
    ...
    *)
    ...
    | Test ->
      ...
      Backend_test.Gen.gen_test el il al elab_env;
    );
    log "Complete."
  with
