
'use strict';

if (typeof console === 'undefined') {
  var Console = function () {
      this.log = function(msg) { debug(msg); };
  };
  var console = new Console();
}

let hostrefs = {};
let hostsym = Symbol("hostref");
function hostref(s) {
  if (! (s in hostrefs)) hostrefs[s] = {[hostsym]: s};
  return hostrefs[s];
}
function eq_ref(x, y) {
  return x === y ? 1 : 0;
}

let spectest = {
  hostref: hostref,
  eq_ref: eq_ref,
  print: console.log.bind(console),
  print_i32: console.log.bind(console),
  print_i64: console.log.bind(console),
  print_i32_f32: console.log.bind(console),
  print_f64_f64: console.log.bind(console),
  print_f32: console.log.bind(console),
  print_f64: console.log.bind(console),
  global_i32: 666,
  global_i64: 666n,
  global_f32: 666.6,
  global_f64: 666.6,
  table: new WebAssembly.Table({initial: 10, maximum: 20, element: 'anyfunc'}),
  memory: new WebAssembly.Memory({initial: 1, maximum: 2})
};

let handler = {
  get(target, prop) {
    return (prop in target) ?  target[prop] : {};
  }
};
let registry = new Proxy({spectest}, handler);

function register(name, instance) {
  registry[name] = instance.exports;
}

function module(bytes, valid = true) {
  let buffer = new ArrayBuffer(bytes.length);
  let view = new Uint8Array(buffer);
  for (let i = 0; i < bytes.length; ++i) {
    view[i] = bytes.charCodeAt(i);
  }
  let validated;
  try {
    validated = WebAssembly.validate(buffer);
  } catch (e) {
    throw new Error("Wasm validate throws");
  }
  if (validated !== valid) {
    if (!validated) WebAssembly.compile(buffer).catch(e => console.log(e));
    throw new Error("Wasm validate failure" + (valid ? "" : " expected"));
  }
  return new WebAssembly.Module(buffer);
}

function instance(mod, imports = registry) {
  return new WebAssembly.Instance(mod, imports);
}

function call(instance, name, args) {
  return instance.exports[name](...args);
}

function get(instance, name) {
  let v = instance.exports[name];
  return (v instanceof WebAssembly.Global) ? v.value : v;
}

function exports(instance) {
  return {module: instance.exports, spectest: spectest};
}

function run(action) {
  action();
}

function assert_malformed(bytes) {
  try { module(bytes, false) } catch (e) {
    if (e instanceof WebAssembly.CompileError) return;
  }
  throw new Error("Wasm decoding failure expected");
}

function assert_malformed_custom(bytes) {
  return;
}

function assert_invalid(bytes) {
  try { module(bytes, false) } catch (e) {
    if (e instanceof WebAssembly.CompileError) return;
  }
  throw new Error("Wasm validation failure expected");
}

function assert_invalid_custom(bytes) {
  return;
}

function assert_unlinkable(mod) {
  try { new WebAssembly.Instance(mod, registry) } catch (e) {
    if (e instanceof WebAssembly.LinkError) return;
  }
  throw new Error("Wasm linking failure expected");
}

function assert_uninstantiable(mod) {
  try { new WebAssembly.Instance(mod, registry) } catch (e) {
    if (e instanceof WebAssembly.RuntimeError) return;
    throw new Error("Wasm trap expected, but got: " + e);
  }
  throw new Error("Wasm trap expected");
}

function assert_uninstantiable_inlined(bytes) {
  let mod = module(bytes);
  assert_uninstantiable(mod);
}

function assert_trap(action) {
  try { action() } catch (e) {
    if (e instanceof WebAssembly.RuntimeError) return;
    throw new Error("Wasm trap expected, but got: " + e);
  }
  throw new Error("Wasm trap expected");
}

function assert_exception(action) {
  try { action() } catch (e) { return; }
  throw new Error("exception expected");
}

let StackOverflow;
try { (function f() { 1 + f() })() } catch (e) { StackOverflow = e.constructor }

function assert_exhaustion(action) {
  try { action() } catch (e) {
    if (e instanceof StackOverflow) return;
  }
  throw new Error("Wasm resource exhaustion expected");
}

function assert_return(action, ...expected) {
  let actual = action();
  if (actual === undefined) {
    actual = [];
  } else if (!Array.isArray(actual)) {
    actual = [actual];
  }
  if (actual.length !== expected.length) {
    throw new Error(expected.length + " value(s) expected, got " + actual.length);
  }
  for (let i = 0; i < actual.length; ++i) {
    switch (expected[i]) {
      case "nan:canonical":
      case "nan:arithmetic":
      case "nan:any":
        // Note that JS can't reliably distinguish different NaN values,
        // so there's no good way to test that it's a canonical NaN.
        if (!Number.isNaN(actual[i])) {
          throw new Error("Wasm NaN return value expected, got " + actual[i]);
        };
        return;
      case "ref.i31":
        if (typeof actual[i] !== "number" || (actual[i] & 0x7fffffff) !== actual[i]) {
          throw new Error("Wasm i31 return value expected, got " + actual[i]);
        };
        return;
      case "ref.any":
      case "ref.eq":
      case "ref.struct":
      case "ref.array":
        // For now, JS can't distinguish exported Wasm GC values,
        // so we only test for object.
        if (typeof actual[i] !== "object") {
          throw new Error("Wasm object return value expected, got " + actual[i]);
        };
        return;
      case "ref.func":
        if (typeof actual[i] !== "function") {
          throw new Error("Wasm function return value expected, got " + actual[i]);
        };
        return;
      case "ref.extern":
        if (actual[i] === null) {
          throw new Error("Wasm reference return value expected, got " + actual[i]);
        };
        return;
      case "ref.null":
        if (actual[i] !== null) {
          throw new Error("Wasm null return value expected, got " + actual[i]);
        };
        return;
      default:
        if (!Object.is(actual[i], expected[i])) {
          throw new Error("Wasm return value " + expected[i] + " expected, got " + actual[i]);
        };
    }
  }
}

// memory_trap1.wast:1
let $$1 = module("");

// memory_trap1.wast:1
let $1 = instance($$1);

// memory_trap1.wast:79
assert_trap(() => call($1, "i32.store", [65_536, 0]));

// memory_trap1.wast:80
assert_trap(() => call($1, "i32.store", [65_535, 0]));

// memory_trap1.wast:81
assert_trap(() => call($1, "i32.store", [65_534, 0]));

// memory_trap1.wast:82
assert_trap(() => call($1, "i32.store", [65_533, 0]));

// memory_trap1.wast:83
assert_trap(() => call($1, "i32.store", [-1, 0]));

// memory_trap1.wast:84
assert_trap(() => call($1, "i32.store", [-2, 0]));

// memory_trap1.wast:85
assert_trap(() => call($1, "i32.store", [-3, 0]));

// memory_trap1.wast:86
assert_trap(() => call($1, "i32.store", [-4, 0]));

// memory_trap1.wast:87
assert_trap(() => call($1, "i64.store", [65_536, 0n]));

// memory_trap1.wast:88
assert_trap(() => call($1, "i64.store", [65_535, 0n]));

// memory_trap1.wast:89
assert_trap(() => call($1, "i64.store", [65_534, 0n]));

// memory_trap1.wast:90
assert_trap(() => call($1, "i64.store", [65_533, 0n]));

// memory_trap1.wast:91
assert_trap(() => call($1, "i64.store", [65_532, 0n]));

// memory_trap1.wast:92
assert_trap(() => call($1, "i64.store", [65_531, 0n]));

// memory_trap1.wast:93
assert_trap(() => call($1, "i64.store", [65_530, 0n]));

// memory_trap1.wast:94
assert_trap(() => call($1, "i64.store", [65_529, 0n]));

// memory_trap1.wast:95
assert_trap(() => call($1, "i64.store", [-1, 0n]));

// memory_trap1.wast:96
assert_trap(() => call($1, "i64.store", [-2, 0n]));

// memory_trap1.wast:97
assert_trap(() => call($1, "i64.store", [-3, 0n]));

// memory_trap1.wast:98
assert_trap(() => call($1, "i64.store", [-4, 0n]));

// memory_trap1.wast:99
assert_trap(() => call($1, "i64.store", [-5, 0n]));

// memory_trap1.wast:100
assert_trap(() => call($1, "i64.store", [-6, 0n]));

// memory_trap1.wast:101
assert_trap(() => call($1, "i64.store", [-7, 0n]));

// memory_trap1.wast:102
assert_trap(() => call($1, "i64.store", [-8, 0n]));

// memory_trap1.wast:103
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.store", [65_536, 0.]))

// memory_trap1.wast:104
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.store", [65_535, 0.]))

// memory_trap1.wast:105
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.store", [65_534, 0.]))

// memory_trap1.wast:106
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.store", [65_533, 0.]))

// memory_trap1.wast:107
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.store", [-1, 0.]))

// memory_trap1.wast:108
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.store", [-2, 0.]))

// memory_trap1.wast:109
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.store", [-3, 0.]))

// memory_trap1.wast:110
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.store", [-4, 0.]))

// memory_trap1.wast:111
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [65_536, 0.]))

// memory_trap1.wast:112
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [65_535, 0.]))

// memory_trap1.wast:113
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [65_534, 0.]))

// memory_trap1.wast:114
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [65_533, 0.]))

// memory_trap1.wast:115
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [65_532, 0.]))

// memory_trap1.wast:116
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [65_531, 0.]))

// memory_trap1.wast:117
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [65_530, 0.]))

// memory_trap1.wast:118
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [65_529, 0.]))

// memory_trap1.wast:119
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [-1, 0.]))

// memory_trap1.wast:120
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [-2, 0.]))

// memory_trap1.wast:121
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [-3, 0.]))

// memory_trap1.wast:122
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [-4, 0.]))

// memory_trap1.wast:123
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [-5, 0.]))

// memory_trap1.wast:124
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [-6, 0.]))

// memory_trap1.wast:125
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [-7, 0.]))

// memory_trap1.wast:126
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [-8, 0.]))

// memory_trap1.wast:127
assert_trap(() => call($1, "i32.store8", [65_536, 0]));

// memory_trap1.wast:128
assert_trap(() => call($1, "i32.store8", [-1, 0]));

// memory_trap1.wast:129
assert_trap(() => call($1, "i32.store16", [65_536, 0]));

// memory_trap1.wast:130
assert_trap(() => call($1, "i32.store16", [65_535, 0]));

// memory_trap1.wast:131
assert_trap(() => call($1, "i32.store16", [-1, 0]));

// memory_trap1.wast:132
assert_trap(() => call($1, "i32.store16", [-2, 0]));

// memory_trap1.wast:133
assert_trap(() => call($1, "i64.store8", [65_536, 0n]));

// memory_trap1.wast:134
assert_trap(() => call($1, "i64.store8", [-1, 0n]));

// memory_trap1.wast:135
assert_trap(() => call($1, "i64.store16", [65_536, 0n]));

// memory_trap1.wast:136
assert_trap(() => call($1, "i64.store16", [65_535, 0n]));

// memory_trap1.wast:137
assert_trap(() => call($1, "i64.store16", [-1, 0n]));

// memory_trap1.wast:138
assert_trap(() => call($1, "i64.store16", [-2, 0n]));

// memory_trap1.wast:139
assert_trap(() => call($1, "i64.store32", [65_536, 0n]));

// memory_trap1.wast:140
assert_trap(() => call($1, "i64.store32", [65_535, 0n]));

// memory_trap1.wast:141
assert_trap(() => call($1, "i64.store32", [65_534, 0n]));

// memory_trap1.wast:142
assert_trap(() => call($1, "i64.store32", [65_533, 0n]));

// memory_trap1.wast:143
assert_trap(() => call($1, "i64.store32", [-1, 0n]));

// memory_trap1.wast:144
assert_trap(() => call($1, "i64.store32", [-2, 0n]));

// memory_trap1.wast:145
assert_trap(() => call($1, "i64.store32", [-3, 0n]));

// memory_trap1.wast:146
assert_trap(() => call($1, "i64.store32", [-4, 0n]));

// memory_trap1.wast:147
assert_trap(() => call($1, "i32.load", [65_536]));

// memory_trap1.wast:148
assert_trap(() => call($1, "i32.load", [65_535]));

// memory_trap1.wast:149
assert_trap(() => call($1, "i32.load", [65_534]));

// memory_trap1.wast:150
assert_trap(() => call($1, "i32.load", [65_533]));

// memory_trap1.wast:151
assert_trap(() => call($1, "i32.load", [-1]));

// memory_trap1.wast:152
assert_trap(() => call($1, "i32.load", [-2]));

// memory_trap1.wast:153
assert_trap(() => call($1, "i32.load", [-3]));

// memory_trap1.wast:154
assert_trap(() => call($1, "i32.load", [-4]));

// memory_trap1.wast:155
assert_trap(() => call($1, "i64.load", [65_536]));

// memory_trap1.wast:156
assert_trap(() => call($1, "i64.load", [65_535]));

// memory_trap1.wast:157
assert_trap(() => call($1, "i64.load", [65_534]));

// memory_trap1.wast:158
assert_trap(() => call($1, "i64.load", [65_533]));

// memory_trap1.wast:159
assert_trap(() => call($1, "i64.load", [65_532]));

// memory_trap1.wast:160
assert_trap(() => call($1, "i64.load", [65_531]));

// memory_trap1.wast:161
assert_trap(() => call($1, "i64.load", [65_530]));

// memory_trap1.wast:162
assert_trap(() => call($1, "i64.load", [65_529]));

// memory_trap1.wast:163
assert_trap(() => call($1, "i64.load", [-1]));

// memory_trap1.wast:164
assert_trap(() => call($1, "i64.load", [-2]));

// memory_trap1.wast:165
assert_trap(() => call($1, "i64.load", [-3]));

// memory_trap1.wast:166
assert_trap(() => call($1, "i64.load", [-4]));

// memory_trap1.wast:167
assert_trap(() => call($1, "i64.load", [-5]));

// memory_trap1.wast:168
assert_trap(() => call($1, "i64.load", [-6]));

// memory_trap1.wast:169
assert_trap(() => call($1, "i64.load", [-7]));

// memory_trap1.wast:170
assert_trap(() => call($1, "i64.load", [-8]));

// memory_trap1.wast:171
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.load", [65_536]))

// memory_trap1.wast:172
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.load", [65_535]))

// memory_trap1.wast:173
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.load", [65_534]))

// memory_trap1.wast:174
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.load", [65_533]))

// memory_trap1.wast:175
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.load", [-1]))

// memory_trap1.wast:176
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.load", [-2]))

// memory_trap1.wast:177
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.load", [-3]))

// memory_trap1.wast:178
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.load", [-4]))

// memory_trap1.wast:179
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.load", [65_536]))

// memory_trap1.wast:180
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.load", [65_535]))

// memory_trap1.wast:181
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.load", [65_534]))

// memory_trap1.wast:182
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.load", [65_533]))

// memory_trap1.wast:183
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.load", [65_532]))

// memory_trap1.wast:184
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.load", [65_531]))

// memory_trap1.wast:185
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.load", [65_530]))

// memory_trap1.wast:186
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.load", [65_529]))

// memory_trap1.wast:187
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.load", [-1]))

// memory_trap1.wast:188
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.load", [-2]))

// memory_trap1.wast:189
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.load", [-3]))

// memory_trap1.wast:190
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.load", [-4]))

// memory_trap1.wast:191
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.load", [-5]))

// memory_trap1.wast:192
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.load", [-6]))

// memory_trap1.wast:193
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.load", [-7]))

// memory_trap1.wast:194
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.load", [-8]))

// memory_trap1.wast:195
assert_trap(() => call($1, "i32.load8_s", [65_536]));

// memory_trap1.wast:196
assert_trap(() => call($1, "i32.load8_s", [-1]));

// memory_trap1.wast:197
assert_trap(() => call($1, "i32.load8_u", [65_536]));

// memory_trap1.wast:198
assert_trap(() => call($1, "i32.load8_u", [-1]));

// memory_trap1.wast:199
assert_trap(() => call($1, "i32.load16_s", [65_536]));

// memory_trap1.wast:200
assert_trap(() => call($1, "i32.load16_s", [65_535]));

// memory_trap1.wast:201
assert_trap(() => call($1, "i32.load16_s", [-1]));

// memory_trap1.wast:202
assert_trap(() => call($1, "i32.load16_s", [-2]));

// memory_trap1.wast:203
assert_trap(() => call($1, "i32.load16_u", [65_536]));

// memory_trap1.wast:204
assert_trap(() => call($1, "i32.load16_u", [65_535]));

// memory_trap1.wast:205
assert_trap(() => call($1, "i32.load16_u", [-1]));

// memory_trap1.wast:206
assert_trap(() => call($1, "i32.load16_u", [-2]));

// memory_trap1.wast:207
assert_trap(() => call($1, "i64.load8_s", [65_536]));

// memory_trap1.wast:208
assert_trap(() => call($1, "i64.load8_s", [-1]));

// memory_trap1.wast:209
assert_trap(() => call($1, "i64.load8_u", [65_536]));

// memory_trap1.wast:210
assert_trap(() => call($1, "i64.load8_u", [-1]));

// memory_trap1.wast:211
assert_trap(() => call($1, "i64.load16_s", [65_536]));

// memory_trap1.wast:212
assert_trap(() => call($1, "i64.load16_s", [65_535]));

// memory_trap1.wast:213
assert_trap(() => call($1, "i64.load16_s", [-1]));

// memory_trap1.wast:214
assert_trap(() => call($1, "i64.load16_s", [-2]));

// memory_trap1.wast:215
assert_trap(() => call($1, "i64.load16_u", [65_536]));

// memory_trap1.wast:216
assert_trap(() => call($1, "i64.load16_u", [65_535]));

// memory_trap1.wast:217
assert_trap(() => call($1, "i64.load16_u", [-1]));

// memory_trap1.wast:218
assert_trap(() => call($1, "i64.load16_u", [-2]));

// memory_trap1.wast:219
assert_trap(() => call($1, "i64.load32_s", [65_536]));

// memory_trap1.wast:220
assert_trap(() => call($1, "i64.load32_s", [65_535]));

// memory_trap1.wast:221
assert_trap(() => call($1, "i64.load32_s", [65_534]));

// memory_trap1.wast:222
assert_trap(() => call($1, "i64.load32_s", [65_533]));

// memory_trap1.wast:223
assert_trap(() => call($1, "i64.load32_s", [-1]));

// memory_trap1.wast:224
assert_trap(() => call($1, "i64.load32_s", [-2]));

// memory_trap1.wast:225
assert_trap(() => call($1, "i64.load32_s", [-3]));

// memory_trap1.wast:226
assert_trap(() => call($1, "i64.load32_s", [-4]));

// memory_trap1.wast:227
assert_trap(() => call($1, "i64.load32_u", [65_536]));

// memory_trap1.wast:228
assert_trap(() => call($1, "i64.load32_u", [65_535]));

// memory_trap1.wast:229
assert_trap(() => call($1, "i64.load32_u", [65_534]));

// memory_trap1.wast:230
assert_trap(() => call($1, "i64.load32_u", [65_533]));

// memory_trap1.wast:231
assert_trap(() => call($1, "i64.load32_u", [-1]));

// memory_trap1.wast:232
assert_trap(() => call($1, "i64.load32_u", [-2]));

// memory_trap1.wast:233
assert_trap(() => call($1, "i64.load32_u", [-3]));

// memory_trap1.wast:234
assert_trap(() => call($1, "i64.load32_u", [-4]));

// memory_trap1.wast:237
assert_return(() => call($1, "i64.load", [65_528]), 7_523_094_288_207_667_809n);

// memory_trap1.wast:238
assert_return(() => call($1, "i64.load", [0]), 7_523_094_288_207_667_809n);

// memory_trap1.wast:242
assert_return(() => call($1, "i64.store", [65_528, 0n]));

// memory_trap1.wast:243
assert_trap(() => call($1, "i32.store", [65_533, 305_419_896]));

// memory_trap1.wast:244
assert_return(() => call($1, "i32.load", [65_532]), 0);

// memory_trap1.wast:245
assert_trap(() => call($1, "i64.store", [65_529, 1_311_768_467_294_899_695n]));

// memory_trap1.wast:246
assert_return(() => call($1, "i64.load", [65_528]), 0n);

// memory_trap1.wast:247
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f32.store", [65_533, 305419904.]))

// memory_trap1.wast:248
run(() => call(instance(module(""), exports($1)),  "run", []));  // assert_return(() => call($1, "f32.load", [65_532]), 0.)

// memory_trap1.wast:249
assert_trap(() => call(instance(module(""), exports($1)),  "run", []));  // assert_trap(() => call($1, "f64.store", [65_529, 1.31176846729e+18]))

// memory_trap1.wast:250
run(() => call(instance(module(""), exports($1)),  "run", []));  // assert_return(() => call($1, "f64.load", [65_528]), 0.)
